...
import type { ReceiptDataForPDF, SchoolInfoForPDF } from '@/lib/receipt-html-template';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    // ── Parse body ────────────────────────────────────────────────────────
    let body: { receiptData?: ReceiptDataForPDF; isAdmin?: boolean; action?: 'print' | 'download' };
    try {
    ...
    const logoUrl = typeof schoolRaw.logoUrl === 'string' ? schoolRaw.logoUrl : null;
    if (logoUrl) {
        try {
    ...
    const disposition = action === 'print'
