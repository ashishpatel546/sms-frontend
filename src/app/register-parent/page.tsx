...
export default function RegisterParentPage() {
    ...
    return (
        <div className="min-h-dvh ink-field flex items-center justify-center p-4">
                    ...
                    {(["mobile", "otp", "students", "details"] as Step[]).map((s, i) => {
                        ...
                        return (
                            <div key={s} className="flex items-center gap-2">
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${done ? "bg-green-500 text-white" : active ? "bg-indigo-600 text-white" : "bg-walnut-800 text-brass-100/55"}`}>
                                    {done ? "✓" : i + 1}
                                </div>
                                {i < 3 && <div className={`w-8 h-0.5 ${done ? "bg-green-500" : "bg-walnut-800"}`} />}
                            </div>
                        );
                    })}
                </div>

                <div className="bg-walnut-850 border border-white/10 rounded-2xl p-6 shadow-2xl">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-walnut-800 border border-white/12 text-white placeholder-brass-100/40 rounded-xl px-4 py-3 pl-10 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
                                    />
                                    {mobileCheck.status === "checking" && (
                                        <div className="absolute inset-y-0 right-3 flex items-center">
                                            <svg className="animate-spin w-4 h-4 text-brass-100/70" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
                                        </div>
                                    )}
                                </div>
