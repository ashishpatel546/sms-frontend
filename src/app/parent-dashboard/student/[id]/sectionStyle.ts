 ...
 *
 * 1. **Fixed gradients from Tailwind's default palette.** Every other colour in
 *    this app comes from the semantic tokens, which `[data-palette='assembly']`
 *    re-tints wholesale. These chips must NOT re-tint — they are app icons, and
 *    a parent learns them by colour, so orange has to stay orange whichever
 *    palette and theme the school picked. Tailwind's stock ramps (`emerald`,
 *    `violet`…) are the only ones `globals.css` never redefines, which is
 *    exactly why they are used here and nowhere else.
 *
...
export const SECTION_STYLE: Record<SectionKey, SectionStyle> = {
  ...
  'ai-tutor': {
    label: 'AI tutor',
    gradient: 'bg-linear-to-br from-fuchsia-500 to-fuchsia-700',
  ...
  library: {
    label: 'Library',
    gradient: 'bg-linear-to-br from-teal-600 to-teal-800',
  ...
  leaves: {
    label: 'Leaves',
    gradient: 'bg-linear-to-br from-cyan-600 to-cyan-700',
  ...
  'id-card': {
    label: 'ID card',
    gradient: 'bg-linear-to-br from-amber-600 to-amber-800',
  ...
  holidays: {
    label: 'Holidays',
    gradient: 'bg-linear-to-br from-sky-600 to-sky-800',
  ...
  info: {
    label: 'Profile',
    gradient: 'bg-linear-to-br from-pink-500 to-pink-800',
