...
export default function ExamScheduleParentView({ classId, sessionId }: ExamScheduleParentViewProps) {
    ...
    if (!schedules || schedules.length === 0) {
        return (
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-12 text-center border border-slate-200 shadow-soft">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {schedules.map((schedule: any) => (
                        <div 
                            ...
                            className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-slate-200 hover:border-brand/40 cursor-pointer transition-all hover:shadow-lg hover:shadow-brand/10 group"
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div className="bg-brand/10 p-3 rounded-lg text-brand group-hover:bg-brand group-hover:text-white transition-colors">
...
function ScheduleMatrix({ scheduleId, classId, onBack }: { scheduleId: number, classId: number, onBack: () => void }) {
    ...
    return (
        <div ref={printRef} className="bg-surface rounded-2xl border border-slate-200/50 overflow-hidden shadow-xl">
            {/* Header */}
            <div className="p-6 border-b border-slate-200/50 flex items-center gap-4">
                <button onClick={onBack} className="p-2 bg-slate-100 hover:bg-slate-100 text-ink rounded-full transition-colors">
                ...
                <div className="text-right text-sm text-ink-muted hidden sm:flex flex-col items-end gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-slate-100 hover:bg-slate-100 text-ink transition-colors disabled:opacity-50 no-print"
                    >
                        {isDownloading ? (
            ...
            {/* Month navigation */}
            <div className="px-4 py-3 border-b border-slate-200/50 flex items-center justify-between no-print">
                <button
                    onClick={goPrev}
                    disabled={!canGoPrev}
                    className="p-2 rounded-lg bg-slate-100 hover:bg-slate-100 text-ink disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
                >
            ...
            <div className="p-4">
                ...
                <div className="grid grid-cols-7 gap-1">
                    {cells.map((dateStr, idx) => {
                        ...
                        let cellBg = "bg-surface-glass border-white/10";
                        if (!inRange)        cellBg = "bg-transparent border-white/5";
                        else if (entry)      cellBg = "bg-brand/10 border-brand/30";
                        else if (holiday)    cellBg = "bg-amber-500/10 border-amber-400/40";
                        else if (isSunday)   cellBg = "bg-surface-secondary/50 border-white/10";
                        ...
                        return (
                            <div
                                ...
                                className={`min-h-20 rounded-lg border p-1.5 flex flex-col transition-colors ${cellBg} ${isToday ? "ring-2 ring-brand ring-offset-1 ring-offset-transparent" : ""} ${inRange && (entry || holiday) ? "cursor-pointer hover:border-brand/80 hover:bg-brand/5" : ""}`}
                            >
                                {/* Date number — Sunday is always rose-red, even when also a holiday */}
                                    ...
                                    <div className="flex flex-col gap-0.5 mt-1">
                                        {entry && (
                                            <div className="bg-brand/10 border border-brand/30 rounded px-1.5 py-1 text-xs leading-tight">
                                                ...
                                                {entry.notes && (
                                                    <div className="text-brand/40 mt-0.5 truncate text-[10px]">{entry.notes}</div>
                                                )}
                                            </div>
                                        )}
                                        {holiday && (
                            ...
                            {selectedDateDetail.entry && (
                                <div className="bg-brand/5 border border-brand/20 rounded-xl p-4">
                                    ...
                                    {selectedDateDetail.entry.notes && (
                                        <div className="mt-3 text-sm text-ink-muted bg-slate-50 p-3 rounded-lg border border-slate-200">
                                            {selectedDateDetail.entry.notes}
                                        </div>
                                    )}
                                </div>
