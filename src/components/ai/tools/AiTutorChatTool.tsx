...
interface AiTutorChatToolProps {
  defaultGrade?: string;
  onUpgradeClick?: () => void;
  /** Overrides the chat panel height — default fills the staff dashboard viewport. */
  heightClass?: string;
}
...
export function AiTutorChatTool({ defaultGrade, onUpgradeClick, heightClass }: AiTutorChatToolProps) {
  ...
  return (
    <FeatureGate feature="chat" onUpgradeClick={onUpgradeClick}>
    <div className={`flex flex-col ${heightClass ?? "h-[calc(100dvh-3.5rem)]"} max-w-3xl mx-auto px-4 py-4`}>
      {/* Header */}
