...
```bash
npm run dev      # next dev -p 4000 — local dev server (port pinned in package.json; local dev only)
...
- **Production and staging use different ports and hostnames.** They come from the deploy pipeline (`.github/workflows/`, `ecosystem.config.js`, AWS SSM parameters and the proxy on the EC2 host) — see `.github/workflows/deploy-ec2.yml` and `ecosystem.config.js`. Never infer a production port from the local map, or the local map from production.
...
Local dev is exposed to the internet through a named Cloudflare tunnel (`home-app`) so the app can be tested on real phones/tablets (PWA install, push notifications, camera/QR flows), not just a desktop browser. Config lives at `~/.cloudflared/config.yml`; the shell profile (`~/.zshrc`) provides `c-tunnel` (run the tunnel) and `cloudflared-config` (open the config in VS Code). Ingress map (specific hostnames must stay ABOVE the `*.appme.in` wildcard — cloudflared matches in order):

| Hostname | Local service |
|---|---|
| `*.appme.in` wildcard (e.g. `edusphere.appme.in`), also `myrealapp.appme.in` | **sms-frontend (this app)** — `localhost:4000` (subdomain doubles as the tenant slug — see `getSchoolSlug()`) |
...
- **AI Assistant (sms-agent)** is separate from school-ai: `src/lib/assistant/client.ts` exchanges the session at `POST {API_URL}/agent/session` for a 30-minute assistant token and calls `AGENT_API_URL` directly (SSE chat, Confirm / Cancel of drafted changes, voice). The UI lives in `src/components/assistant/`: `AssistantProvider` (shown only when the school has the `ai_agent` feature and the user is staff; Ctrl/⌘+J), `AssistantLauncher` (top bar), `AssistantPanel` (a sheet on phones, a right-hand panel on tablet and up) and `DraftCard` (dashed "pencilled in" until confirmed, then "inked" sage). A conversation is the backend's assistant session: token refreshes resume it (`{ resume }`), "New chat" ends it, and one that went idle past the school's idle limit (hub setting) comes back as a new session, so the panel starts fresh. Voice follows the school's hub settings (`caps.voice.input` / `output`: `off` hides the mic or read-aloud control, `device` uses the browser's own speech, `server` uses sms-agent's speech models); server speech falls back to the browser's own speech recognition and synthesis when unavailable (`src/lib/assistant/voice.ts`).
- Env vars: `AGENT_API_URL` is the browser-reachable URL of sms-agent (local `http://localhost:4030`). `AI_API_URL` is the public/tunneled URL of school-ai reachable from the browser (not `localhost` in anything but same-machine dev); must be CORS-allowlisted on the school-ai side.
